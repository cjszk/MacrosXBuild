...
class Graph extends React.Component {
    ...
    renderChart() {
        ...
        let sliderData;
        let graphData;
        let macros = this.filterDataByDate(data.entries).map((item, i) => {
            return {
                    fat: item.fat,
        ...
        for (let i=0; i<macros.length; i++) {
            if (!Object.keys(newData).includes([macros[i].date])) {
                newData[macros[i].date] = {
                    key: i,
                    date: macros[i].date,
                    carbs: 0,
                    protein: 0,
            ...
            newData[macros[i].date].fat = newData[macros[i].date].fat + macros[i].fat*9;
            newData[macros[i].date].protein = newData[macros[i].date].protein + macros[i].protein*4;
            newData[macros[i].date].carbs = newData[macros[i].date].carbs + macros[i].carbs*4;
            newData[macros[i].date].calories = newData[macros[i].date].calories + macros[i].fat*9 + macros[i].protein*4 + macros[i].carbs*4;
        }
        macroData = [];
